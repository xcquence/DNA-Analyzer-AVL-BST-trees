...
class AvlTree
{
  public:
    ...
    /*
    * Extra public functions , find(const Comparable &x)
    * NumberOfNodes(), InternalPathLength(); these functions
    * will find specific nodes, calculate internal path lengths, and
  ...
  private:
    ...
    /*
    * Private functions that are created, find(Comparable &x, AvlNode *node),
    NumberOfNodes(AvlNode *node), InternalPathLength(int length_value, AvlNode *node)
    */
    ...
    int NumberOfNodes(AvlNode *node) const {
        if(node == nullptr){
          return 0;
        }
        else{
    ...
    int InternalPathLength(int length_value, AvlNode* node) const {
        if(node == nullptr){
            return 0;
        }
     ...
     */
    void remove( const Comparable &x, AvlNode* node, int &SuccessCalls, bool &isRemoved){
        if(node == nullptr){
          return;
        }
        if(x < node->element){
            SuccessCalls++;
            remove(x, node->left, SuccessCalls, isRemoved);
        }
        else if(node->element < x){
            SuccessCalls++;
            remove(x, node->left, SuccessCalls, isRemoved);
            ...
            remove(node->element, node->right, SuccessCalls, isRemoved);
        }
        else{
            AvlNode *temp = node;
     ...
     */
    bool contains( const Comparable & x, AvlNode *t , int &SuccessCalls) const
    {
        if( t == nullptr )
            return false;
        else if( x < t->element ){
